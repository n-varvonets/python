...
Таким образом на выполнение потоков еще накладывается и задержка на:
    - блокировку;
    - на освобождение muteX;
    - захват muteX другим потоком...
"""
...
task.join()  # join -заблокирует интрепритатор на данной строке  до тех пор пока тело функции handler не завершит свою работу
