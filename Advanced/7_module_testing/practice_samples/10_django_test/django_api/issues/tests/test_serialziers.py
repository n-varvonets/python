...
class LoginSerializerTestCase(TestCase):
    ...
    def test_validate_with_wrong_credentials(self):
        ...
        self.assertFalse(serializer.is_valid())  # проверяем корректность
        self.assertGreater(len(serializer.errors), 0)  # проверяем кол-во ошибок
        self.assertRegex(serializer.errors['non_field_errors'][0],
                         'Incorrect username\/password')   # и какие ошибки у нас вывелись
