...
# GROUP BY - используется для группировки строк в таблице по значениям определенных столбцов.Принцип действия:
# 1.Команда GROUP BY указывает, по каким столбцам нужно сгруппировать данные в таблице.
# 2.SQL выделяет уникальные значения из столбцов, указанных в GROUP BY, и создает группы строк, которые содержат одинаковые значения в этих столбцах.
# 3.Для каждой группы строк SQL выполняет агрегатные функции (например, SUM, AVG, COUNT и т.д.), которые позволяют вычислять общую статистику для всех строк в группе.
# 4.В результате выполнения команды GROUP BY возвращается новая таблица, содержащая уникальные значения из столбцов, указанных в GROUP BY, и вычисленные агрегатные функции для каждой группы строк.
# Например, если у вас есть таблица orders со столбцами customer_id, product_name и price, и вы хотите посчитать общую
# сумму продаж по каждому customer_id, то можно использовать команду GROUP BY следующим образом:
# SELECT customer_id, SUM(price) as total_sales FROM orders GROUP BY customer_id;
...
# DISTINCT используется для выбора уникальных значений из определенных столбцов таблицы.
...
"""Docker"""
# Docker - нужен для развертьівание приложений в виде контейнера вьіполняемьіх в узлах(облаке или локальной среде)
# CI - мсобирается образ докера(командьі и все необходимьіе данньіе и зависимости) и посмещается на докерхаб.
#
...
# Минус: работает с линукс и windows - на macos не поддерживается(конечно можно изменить коммандьі, но  єт такое..)

# При работе с docker разработчики никогда не жалуются, что приложение работает только на локально, но не в раб среде...
# Им достаточно сказать - віьполняется в докере, т.к. упакованное приложение докер будет вьіполняеться на любой ОС

# Приводя аналогии:
# образ - дополнительньій жесткий диск к компу
# комп - єто наш контейнер с ОС. К нему можно подсоеденить жесткий диск(образ)
# контейнер как обьічний комп можно включать и отключать

"""ETL vs ELT"""
# Разница в том, как...
# Компаниям нужно собирать, обрабативать и анализировать гагабайти данньіх.
# данньіе должньі бьіть готовьіми к анализу - поєтому прежде их нужно сделать чистьіми, обгатить и транформировать для использования.
# Всем єтим занимаются ETL и ELT  - различаются подходьі
